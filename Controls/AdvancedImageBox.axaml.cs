    ...
    public class AdvancedImageBox : UserControl
    {
        ...
        public class ZoomLevelCollection : IList<int>
        {
            ...
            public static ZoomLevelCollection Default =>
                new(Enumerable.Range(1, 1200).ToArray());
            #endregion
        ...
        public static readonly StyledProperty<ObservableCollection<Rect>> RegionsProperty =
            AvaloniaProperty.Register<AdvancedImageBox, ObservableCollection<Rect>>(nameof(Regions), 
                new ObservableCollection<Rect>(), 
                notifying: OnRegionsChanging);

        private static void OnRegionsChanging(IAvaloniaObject sender, bool beforeChanging)
        {
            if (beforeChanging)
                return;

            if (!(sender is AdvancedImageBox imgbox))
                return;

            imgbox.InvalidateVisual();
        }                

        public ObservableCollection<Rect> Regions
        {
        ...
        public override void Render(DrawingContext context)
        {
            ...
            Regions?.ToList().ForEach((r) => {
                var rect = GetOffsetRectangle(r);
                // context.FillRectangle(Avalonia.Media.Brushes.AliceBlue, rect);
                context.DrawRectangle(new Pen(Avalonia.Media.Brushes.AliceBlue), rect);
            });
        }
